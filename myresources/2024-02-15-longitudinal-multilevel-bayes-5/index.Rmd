...
We previously used the `brms` and `rethinking` R packages to run our `Stan` models in `R`, without having to write `Stan` code. The `rethinking` package does not support ODE models, as described in the Statistical Rethinking book, those need to be implemented in `Stan`. While `brms`


This is a tutorial and worked example, to illustrate how one can use 
