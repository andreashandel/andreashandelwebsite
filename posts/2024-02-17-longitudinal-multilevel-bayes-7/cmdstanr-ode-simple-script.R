...
odemod2 <- function(t,y,parms) 
{
  ...
  dlogU = - bet*y[3]
  dlogI = bet*y[1]*y[3]/y[2] - gamm
  dlogV = alph*y[2]/y[3] - et
...
print(R0)
# run the ODE model
oderes <- ode(y = y0, t = t, func = odemod, parms = odeparms)
plot(oderes[,1],log(oderes[,4]),type='l')
...
y0 =  log(c(1e8,1,exp(V0)))
oderes2 <- ode(y = y0, t = t, func = odemod, parms = odeparms)
# look at virus load trajectory
# note that it's already on the log scale
lines(oderes2[,1],log(oderes2[,4]), col='red')
